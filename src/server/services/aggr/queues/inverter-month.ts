...
export function inverterMonth ({ queuer }: ServiceHelpers): void {
  function selectLastLog (item: Inverter): SelectQueryBuilder<unknown> {
    return getConnection('local')
      .createQueryBuilder()
      .select('UNIX_TIMESTAMP(FROM_UNIXTIME(timestamp / 1000, "%Y-%m-1")) * 1000', 'timestamp')
  ...
  queuer.consume<Inverter>('aggr-inverter-month', (item, callback) => {
    selectLastLog(item)
      .execute()
